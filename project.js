...
function getEx() {
    return JSON.parse(localStorage.getItem(LOCAL_STORAGE_EX)) || []
}
...
function updateEx(upEx) {
    ...
    for (const expense of expenses) {
        if (expense.id === +upEx) {
            ...
            document.getElementById('update').value = expense.id
        }
    }
}
...
function displayEx() {
    ...
    for (const expense of getEx()) {
        ...
        <tr class="hover:bg-indigo-50 transition-colors border-b border-gray-100">
            ...
            <td class="p-4 font-bold text-indigo-700 text-lg">${expense.amount} ₪</td>
...
function addFilteredEx() {
    ...
    for (const expense of filteredEx) {
        ...
        <tr class="hover:bg-indigo-50 transition-colors border-b border-gray-100">
            ...
            <td class="p-4 font-bold text-indigo-700">${expense.amount.toLocaleString()} ₪</td>
            <td class="p-4 text-gray-500 font-mono text-sm">${expense.date}</td>
        </tr>
        `
    }
